...
def sum_bin_id(binary_id, n):
    # Convert binary ID to integer and add n
    int_id = int.from_bytes(binary_id, byteorder='big') + n
    # Convert integer ID back to binary
    updated_binary_id = int_id.to_bytes(16, byteorder='big')
    # Return the sum of the integer ID and n and the updated binary ID
    return  updated_binary_id
...
for i in range(NUMBER_OF_USERS):
    ...
    for prod in range(N_PRODUCTS):
        # Generate more products per user
        pid = random_id()
        url = fake.url()
        ...
        datetime0 =  fake.date_between(start_date='-60d', end_date='today')
